# 杀戮尖塔卡牌信息

用于记录卡牌名称、费用和简介。

## 卡牌索引（名称 · ModelId）

与代码 `workspace/mod/ModCode/Cards/*.cs`、本地化 `mod/localization/zhs/cards.json` 的 `{ModelId}.title` 对齐。C# 类名 PascalCase，ModelId 为类名的 `SCREAMING_SNAKE_CASE`。`+` 升级牌不单独列出。

| 显示名 | ModelId | C# 类 | 稀有度 | 类型 |
|--------|---------|-------|--------|------|
| 孤注一掷 | `ALL_IN` | `AllIn` | 罕见 | 技能 |
| 古老死亡怨魂 | `ANCIENT_DEATHS_RANCOR` | `AncientDeathsRancor` | 罕见 | 攻击 |
| 打击 | `BASIC_ATTACK` | `BasicAttack` | 其他 | 攻击 |
| 防御 | `BASIC_DEFENSE` | `BasicDefense` | 其他 | 技能 |
| 夸耀咆哮 | `BATTLE_CRY` | `BattleCry` | 普通 | 技能 |
| 鲜血征收 | `BLOOD_LEVY` | `BloodLevy` | 罕见 | 攻击 |
| 破势如潮 | `BREAKING_MOMENTUM` | `BreakingMomentum` | 稀有 | 能力 |
| 拳套打击 | `CAESTUS_STRIKE` | `CaestusStrike` | 普通 | 攻击 |
| 卡利亚贯刺 | `CARIAN_PIERCER` | `CarianPiercer` | 罕见 | 攻击 |
| 卡利亚式奉还 | `CARIAN_RETRIBUTION` | `CarianRetribution` | 稀有 | 攻击 |
| 卡利亚迅剑 | `CARIAN_SLICER` | `CarianSlicer` | 普通 | 攻击 |
| 蓄力攻击 | `CHARGED_ATTACK` | `ChargedAttack` | 其他 | 攻击 |
| 蓄力重击 | `CHARGED_HEAVY_ATTACK` | `ChargedHeavyAttack` | 先古 | 攻击 |
| 彗星亚兹勒 | `COMET_AZUR` | `CometAzur` | 稀有 | 攻击 |
| 螺旋魔砾 | `SPIRAL_GLINTSTONE` | `SpiralGlintstone` | 罕见 | 攻击 |
| 帚星 | `COMET_SHARD` | `CometShard` | 罕见 | 攻击 |
| 腐败感知 | `CORRUPTION_SENSE` | `CorruptionSense` | 罕见 | 能力 |
| 腐败力量 | `CORRUPTION_STRENGTH` | `CorruptionStrength` | 罕见 | 能力 |
| 爆散结晶 | `CRYSTAL_BURST` | `CrystalBurst` | 罕见 | 攻击 |
| 决心 | `DETERMINATION` | `Determination` | 普通 | 技能 |
| 游刃有余 | `EASY_HANDLING` | `EasyHandling` | 罕见 | 技能 |
| 元素瓶 | `ELEMENT_FLASK` | `ElementFlask` | 衍生 | 技能 |
| 余火 | `EMBER` | `Ember` | 罕见 | 能力 |
| 忍耐 | `ENDURE` | `Endure` | 罕见 | 技能 |
| 黄金树坠落震击 | `ERDTREE_SHOCK` | `ErdtreeShock` | 普通 | 攻击 |
| 致命一击 | `FATAL_STRIKE` | `FatalStrike` | 普通 | 攻击 |
| 火焰啊，净化 | `FLAME_PURIFY` | `FlamePurify` | 普通 | 技能 |
| 垃圾王的庇佑 | `GARBAGE_KING_BLESSING` | `GarbageKingBlessing` | 稀有 | 能力 |
| 特大跳劈 | `GIANT_HUNT` | `GiantHunt` | 普通 | 攻击 |
| 巨剑阵 | `GLINTBLADE_PHALANX` | `GlintbladePhalanx` | 罕见 | 技能 |
| 大块崩裂辉石 | `GLINTSTONE_CHUNK` | `GlintstoneChunk` | 普通 | 攻击 |
| 好运 | `GOOD_LUCK` | `GoodLuck` | 稀有 | 技能 |
| 反击 | `GUARD_COUNTER` | `GuardCounter` | 其他 | 攻击 |
| 荷莱露的撼地 | `HOULOU_GROUND_SLAM` | `HoulouGroundSlam` | 罕见 | 攻击 |
| 恫吓 | `INTIMIDATION` | `Intimidation` | 普通 | 技能 |
| 杰克的酒 | `JACK_WINE` | `JackWine` | 罕见 | 技能 |
| 狮子斩 | `LION_CLAW` | `LionClaw` | 罕见 | 攻击 |
| 罗蕾塔的大弓 | `LORETTAS_GREATBOW` | `LorettasGreatbow` | 罕见 | 攻击 |
| 魔法之境 | `MAGIC_REALM` | `MagicRealm` | 稀有 | 能力 |
| 黑夜彗星 | `NIGHT_COMET` | `NightComet` | 罕见 | 攻击 |
| 掠夺之火 | `PLUNDERING_FIRE` | `PlunderingFire` | 稀有 | 技能 |
| 祈祷一击 | `PRAYER_STRIKE` | `PrayerStrike` | 罕见 | 攻击 |
| 深具睿智 | `PROFOUND_WISDOM` | `ProfoundWisdom` | 罕见 | 技能 |
| 责罚荆棘 | `PUNISHING_THORNS` | `PunishingThorns` | 罕见 | 攻击 |
| 准备架势 | `READY_STANCE` | `ReadyStance` | 罕见 | 攻击 |
| 回归性原理 | `RETURN_PRINCIPLE` | `ReturnPrinciple` | 稀有 | 技能 |
| 岩石球 | `ROCK_BALL` | `RockBall` | 普通 | 攻击 |
| 岩石剑 | `ROCK_BLADE` | `RockBlade` | 普通 | 技能 |
| 猩红代价 | `SCARLET_COST` | `ScarletCost` | 普通 | 技能 |
| 赊账 | `SCARLET_CREDIT` | `ScarletCredit` | 罕见 | 技能 |
| 猩红诱惑 | `SCARLET_TEMPTATION` | `ScarletTemptation` | 稀有 | 能力 |
| 寻找 | `SEARCH` | `Search` | 普通 | 技能 |
| 切腹 | `SEPPUKU` | `Seppuku` | 罕见 | 技能 |
| 盾牌冲击 | `SHIELD_CRASH` | `ShieldCrash` | 普通 | 攻击 |
| 小圆盾弹反 | `SMALL_ROUND_SHIELD_PARRY` | `SmallRoundShieldParry` | 先古 | 技能 |
| 箭步-上砍 | `STAMP_UPPERCUT` | `StampUppercut` | 普通 | 攻击 |
| 唤起风暴 | `STORMCALLER` | `Stormcaller` | 普通 | 攻击 |
| 图腾碑石 | `TOTEM_TABLET` | `TotemTablet` | 稀有 | 攻击 |
| 轻装上阵 | `TRAVEL_LIGHT` | `TravelLight` | 普通 | 技能 |
| 休战 | `TRUCE` | `Truce` | 普通 | 技能 |
| 双刺毒花 | `TWIN_STING_POISON_FLOWER` | `TwinStingPoisonFlower` | 罕见 | 攻击 |
| 卸力 | `UNBURDEN` | `Unburden` | 罕见 | 技能 |
| 乘胜追击 | `VICTORY_RUSH` | `VictoryRush` | 罕见 | 能力 |
| 别急，等我启动 | `WAIT_FOR_ME_TO_START` | `WaitForMeToStart` | 稀有 | 技能 |
| 好胜心 | `WILL_TO_WIN` | `WillToWin` | 稀有 | 能力 |

## 实现状态（代码）

共 **63** 张奖池牌 + **1** 张衍生牌（`ELEMENT_FLASK`）。下表与索引一一对应。

| 显示名 | ModelId | 状态 |
|--------|---------|------|
| 孤注一掷 | `ALL_IN` | 已实现（X 费；选手牌消耗后打出 X 遍） |
| 古老死亡怨魂 | `ANCIENT_DEATHS_RANCOR` | 已实现（含下回合重复） |
| 打击 | `BASIC_ATTACK` | 已实现（含削韧；显示名「打击」） |
| 防御 | `BASIC_DEFENSE` | 已实现 |
| 夸耀咆哮 | `BATTLE_CRY` | 已实现 |
| 鲜血征收 | `BLOOD_LEVY` | 已实现 |
| 破势如潮 | `BREAKING_MOMENTUM` | 已实现 |
| 拳套打击 | `CAESTUS_STRIKE` | 已实现 |
| 卡利亚贯刺 | `CARIAN_PIERCER` | 已实现 |
| 卡利亚式奉还 | `CARIAN_RETRIBUTION` | 已实现（阻止伤害并反击） |
| 卡利亚迅剑 | `CARIAN_SLICER` | 已实现（纯法术；无削韧） |
| 蓄力攻击 | `CHARGED_ATTACK` | 已实现 |
| 蓄力重击 | `CHARGED_HEAVY_ATTACK` | 已实现（先古；由蓄力攻击变化） |
| 彗星亚兹勒 | `COMET_AZUR` | 已实现（选抽牌堆消耗） |
| 螺旋魔砾 | `SPIRAL_GLINTSTONE` | 已实现（% Max HP 魔伤；最少 1；不吃魔力加成） |
| 帚星 | `COMET_SHARD` | 已实现 |
| 腐败感知 | `CORRUPTION_SENSE` | 已实现 |
| 腐败力量 | `CORRUPTION_STRENGTH` | 已实现 |
| 爆散结晶 | `CRYSTAL_BURST` | 已实现（溅射；侧栏规则） |
| 决心 | `DETERMINATION` | 已实现 |
| 游刃有余 | `EASY_HANDLING` | 已实现 |
| 元素瓶 | `ELEMENT_FLASK` | 已实现（衍生；不进牌库/录制清单） |
| 余火 | `EMBER` | 已实现 |
| 忍耐 | `ENDURE` | 已实现 |
| 黄金树坠落震击 | `ERDTREE_SHOCK` | 已实现 |
| 致命一击 | `FATAL_STRIKE` | 已实现 |
| 火焰啊，净化 | `FLAME_PURIFY` | 已实现 |
| 垃圾王的庇佑 | `GARBAGE_KING_BLESSING` | 已实现（虚无） |
| 特大跳劈 | `GIANT_HUNT` | 已实现 |
| 巨剑阵 | `GLINTBLADE_PHALANX` | 已实现 |
| 大块崩裂辉石 | `GLINTSTONE_CHUNK` | 已实现（0 费随机多段；消耗） |
| 好运 | `GOOD_LUCK` | 已实现 |
| 反击 | `GUARD_COUNTER` | 已实现（含充能） |
| 荷莱露的撼地 | `HOULOU_GROUND_SLAM` | 已实现（含下回合追击） |
| 恫吓 | `INTIMIDATION` | 已实现 |
| 杰克的酒 | `JACK_WINE` | 已实现 |
| 狮子斩 | `LION_CLAW` | 已实现 |
| 罗蕾塔的大弓 | `LORETTAS_GREATBOW` | 已实现（溅射；侧栏规则） |
| 魔法之境 | `MAGIC_REALM` | 已实现 |
| 黑夜彗星 | `NIGHT_COMET` | 已实现 |
| 掠夺之火 | `PLUNDERING_FIRE` | 已实现（消耗） |
| 祈祷一击 | `PRAYER_STRIKE` | 已实现 |
| 深具睿智 | `PROFOUND_WISDOM` | 已实现 |
| 责罚荆棘 | `PUNISHING_THORNS` | 已实现 |
| 准备架势 | `READY_STANCE` | 已实现 |
| 回归性原理 | `RETURN_PRINCIPLE` | 已实现 |
| 岩石球 | `ROCK_BALL` | 已实现（多段魔法削韧） |
| 岩石剑 | `ROCK_BLADE` | 已实现 |
| 猩红代价 | `SCARLET_COST` | 已实现 |
| 赊账 | `SCARLET_CREDIT` | 已实现 |
| 猩红诱惑 | `SCARLET_TEMPTATION` | 已实现（每回合选手牌消耗） |
| 寻找 | `SEARCH` | 已实现（升级后 0 费） |
| 切腹 | `SEPPUKU` | 已实现 |
| 盾牌冲击 | `SHIELD_CRASH` | 已实现 |
| 小圆盾弹反 | `SMALL_ROUND_SHIELD_PARRY` | 已实现（先古） |
| 箭步-上砍 | `STAMP_UPPERCUT` | 已实现 |
| 唤起风暴 | `STORMCALLER` | 已实现（X 费多段削韧） |
| 图腾碑石 | `TOTEM_TABLET` | 已实现（含充能） |
| 轻装上阵 | `TRAVEL_LIGHT` | 已实现 |
| 休战 | `TRUCE` | 已实现 |
| 双刺毒花 | `TWIN_STING_POISON_FLOWER` | 已实现 |
| 卸力 | `UNBURDEN` | 已实现 |
| 乘胜追击 | `VICTORY_RUSH` | 已实现（击晕后下一张攻击双倍伤害） |
| 别急，等我启动 | `WAIT_FOR_ME_TO_START` | 已实现 |
| 好胜心 | `WILL_TO_WIN` | 已实现 |

机制缺口见 `implementation_log.md`：卡牌正式卡面、更多魔法牌与数值实战调优等。

## 设计原则（当前）

- 基础/其他牌只有 `打击`（`BASIC_ATTACK`）、`防御`、`蓄力攻击`、`反击`；出生牌组为 4 打击、4 防御、1 蓄力攻击、1 反击。
- 基础牌尽量「不升级也能打」；升级是锦上添花，不是刚需（塔二篝火偏少）。
- `垃圾王的庇佑` 带 **虚无**，避免每回合白送力量叠太高。
- `掠夺之火` 带 **消耗**，避免一回合吸血爆发过强。
- 削韧流派：先削满 → 眩晕 → `致命一击` / `乘胜追击` 收尾。
- 腐败流派：叠层换抽牌/能量/力量 → `火焰啊，净化` / `祈祷一击` / `杰克的酒` 等消化。
- 过牌 + **消耗** → **充能** 回流（如 `反击`、`图腾碑石`）。
- 法师线：**大部分纯法术不加削韧**（法环里多数魔法削韧可忽略）；带「剑/冲击/奉还」等物理成分的卡利亚牌可保留少量削韧，与纯法术区分。

## 目标卡池规模

奖池按 **86 张**（`+` 不单独计）规划。

### 稀有度分布

| 稀有度 | 目标张数 |
|---|---:|
| 普通 | 20 |
| 罕见 | 35 |
| 稀有 | 25 |
| 其他 | 6 |
| 先古 | 2 |

### 费用分布

| 费用 | 目标张数 |
|---|---:|
| 0 费 | 12 |
| 1 费 | 50 |
| 2 费 | 15 |
| 3 费 | 7 |
| X 费 | 2 |

### 类型分布

| 类型 | 目标张数 |
|---|---:|
| 攻击 | 34 |
| 技能 | 34 |
| 能力 | 18 |

## 卡牌模板

```md
### 卡牌名称

- ModelId: `SCREAMING_SNAKE_CASE`（与 C# 类名对应，如 `AllIn` → `ALL_IN`）
- 显示名: 与 `mod/localization/zhs/cards.json` 的 `{ModelId}.title` 一致
- 费用:
- 类型:
- 效果:
- 稀有度:
```
# 效果
猩红腐败 n层: 你的回合开始时失去 n 点生命值。n 的上限是 5。不会自然衰减；卡牌可以减少或清除层数；该失血无视格挡。
充能 n: 开局放入消耗堆。每当你打出其他牌，充能减少 1；减到 0 时该牌回到手牌并可打出。若该牌被强制消耗（未打出），充能重置为 n，留在消耗堆并重新计时（此时无法打出），直至再次减到 0 回手。 
削韧 n: 每段攻击造成 n 点失衡值。敌人的失衡值为 0 时，敌人会进入眩晕状态，然后重置失衡值。
虚无: 如果回合结束时这张牌仍在手牌中，将其消耗
小怪 失衡值 10
爪牙/召唤物 失衡值 10（含 Boss/精英 战内召唤：如 `MinionPower` 单位、同族神宫信徒、感染虫死后刷新的眩晕小虫等；不按 Boss/精英 房间抬高）
精英 失衡值 15-20不等
    群怪精英 15
    单个精英 20
boss 失衡值 25

魔力 n: 你的魔力属性攻击牌，每段在结算时对主目标额外造成 n 点伤害（n = 当前魔力层数）。仅主目标段享受此加成。
溅射: 单体选目标的魔力属性攻击可用。每段独立结算：主目标受到 (X + 魔力) 点伤害（X 为牌面该段伤害，含易伤等常规修正）；其余每个敌人受到 floor((X + 魔力) / 2) 点伤害，最少 1。溅射段不再单独叠加力量或魔力。若主目标被击杀，仍按该段造成的完整 (X + 魔力) 计算溅射，不因溢出扣减。仅当场上还有其他敌人时生效。溅射段不打削韧。

冻伤：敌人受到最大生命值7%的伤害（最少一点伤害） 敌人受到的物理伤害增加20% 最少1点伤害 持续两回合 期间无法再次获得冻伤

# 卡牌列表

## 攻击牌

### 打击

- ModelId: `BASIC_ATTACK`
- 费用: 1
- 类型: 攻击
- 效果: 造成6点伤害 削韧 2
- 稀有度: 其他

### 打击+

- 费用: 1
- 类型: 攻击
- 效果: 造成9点伤害 削韧 2
- 稀有度: 其他

### 蓄力攻击
- ModelId: `CHARGED_ATTACK`

- 费用: 2
- 类型: 攻击
- 效果: 造成15点伤害 削韧 5
- 稀有度: 其他

### 蓄力攻击+

- 费用: 2
- 类型: 攻击
- 效果: 造成19点伤害 削韧 6
- 稀有度: 其他

### 蓄力重击
- ModelId: `CHARGED_HEAVY_ATTACK`

- 费用: 1
- 类型: 攻击
- 效果: 造成15点伤害 下一次受伤减免75% 削韧 5
- 稀有度: 先古

### 蓄力重击+

- 费用: 1
- 类型: 攻击
- 效果: 造成19点伤害 下一次受伤减免75% 削韧 6
- 稀有度: 先古

### 反击
- ModelId: `GUARD_COUNTER`

- 费用: 0
- 类型: 攻击
- 效果: 本回合受到的伤害降低50% 造成6点伤害 削韧 5 充能: 5
- 稀有度: 其他

### 反击+

- 费用: 0
- 类型: 攻击
- 效果: 本回合受到的伤害降低50% 造成6点伤害 削韧 6 充能: 4
- 稀有度: 其他

### 唤起风暴
- ModelId: `STORMCALLER`

- 费用: X
- 类型: 攻击
- 效果: 对所有敌人造成3点伤害X+1次 削韧: 2
- 稀有度: 普通

### 唤起风暴+

- 费用: X
- 类型: 攻击
- 效果: 对所有敌人造成4点伤害X+2次 削韧: 2
- 稀有度: 普通

### 大块崩裂辉石
- ModelId: `GLINTSTONE_CHUNK`

- 费用: 0
- 类型: 攻击
- 效果: 对随机敌人造成2点魔力伤害6次 消耗
- 稀有度: 普通

### 大块崩裂辉石+

- 费用: 0
- 类型: 攻击
- 效果: 对随机敌人造成2点魔力伤害8次 消耗
- 稀有度: 普通

### 黄金树坠落震击
- ModelId: `ERDTREE_SHOCK`

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成7点伤害 抽一张牌 削韧 2
- 稀有度: 普通

### 黄金树坠落震击+

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成7点伤害 抽两张牌 削韧 2
- 稀有度: 普通

### 拳套打击
- ModelId: `CAESTUS_STRIKE`

- 费用: 2
- 类型: 攻击
- 效果: 造成4点伤害 削韧 8
- 稀有度: 普通

### 拳套打击+

- 费用: 2
- 类型: 攻击
- 效果: 造成6点伤害 削韧 10
- 稀有度: 普通

### 盾牌冲击
- ModelId: `SHIELD_CRASH`

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成4点伤害 获得6点格挡 削韧 2
- 稀有度: 普通

### 盾牌冲击+

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成6点伤害 获得8点格挡 削韧 2
- 稀有度: 普通

### 致命一击
- ModelId: `FATAL_STRIKE`

- 费用: 0
- 类型: 攻击
- 效果: 如果敌人处于眩晕状态 造9点伤害两次
- 稀有度: 普通

### 致命一击+

- 费用: 0
- 类型: 攻击
- 效果: 保留 如果敌人处于眩晕状态 造成9点伤害两次
- 稀有度: 普通

### 特大跳劈
- ModelId: `GIANT_HUNT`

- 费用: 1
- 类型: 攻击
- 效果: 造成5点伤害两次并给予1层虚弱 削韧 3
- 稀有度: 普通

### 特大跳劈+

- 费用: 1
- 类型: 攻击
- 效果: 造成8点伤害两次并给予1层虚弱 削韧 3
- 稀有度: 普通

### 卡利亚迅剑
- ModelId: `CARIAN_SLICER`

- 费用: 1
- 类型: 攻击
- 效果: 造成6点魔力属性伤害 将词牌返回你的手牌 
- 稀有度: 普通

### 卡利亚迅剑+

- 费用: 1
- 类型: 攻击
- 效果: 造成9点魔力属性伤害 将词牌返回你的手牌 
- 稀有度: 普通

### 箭步-上砍
- ModelId: `STAMP_UPPERCUT`

- 费用: 2
- 类型: 攻击
- 效果: 获得4点格挡 造成6点伤害 削韧 7
- 稀有度: 普通

### 箭步-上砍+

- 费用: 2
- 类型: 攻击
- 效果: 获得6点格挡 造成8点伤害 削韧 7
- 稀有度: 普通

### 岩石球
- ModelId: `ROCK_BALL`

- 费用: 1
- 类型: 攻击
- 效果: 造成2点魔力属性伤害3次 削韧 2
- 稀有度: 普通

### 岩石球+

- 费用: 1
- 类型: 攻击
- 效果: 造成2点魔力属性伤害4次 削韧 2
- 稀有度: 普通

### 冻霜踏地

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成3点魔力属性伤害 冻伤
- 稀有度: 普通

### 冻霜踏地+

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成6点魔力属性伤害 冻伤
- 稀有度: 普通

### 准备架势
- ModelId: `READY_STANCE`

- 费用: 1
- 类型: 攻击
- 效果: 造成4点伤害 获得1点力量 削韧 4
- 稀有度: 罕见

### 准备架势+

- 费用: 1
- 类型: 攻击
- 效果: 造成4点伤害 获得2点力量 削韧 5
- 稀有度: 罕见

### 螺旋魔砾
- ModelId: `SPIRAL_GLINTSTONE`

- 费用: 1
- 类型: 攻击
- 效果: 造成目标10%血量上限的魔法属性伤害（最少1点，不吃魔力加成）
- 稀有度: 罕见

### 螺旋魔砾+

- 费用: 1
- 类型: 攻击
- 效果: 造成目标12%血量上限的魔法属性伤害（最少1点，不吃魔力加成）
- 稀有度: 罕见

### 祈祷一击
- ModelId: `PRAYER_STRIKE`

- 费用: 1
- 类型: 攻击
- 效果: 减少一层猩红腐败 造成4点伤害 削韧 2
- 稀有度: 罕见

### 祈祷一击+

- 费用: 1
- 类型: 攻击
- 效果: 减少两层猩红腐败 造成5点伤害 削韧 2
- 稀有度: 罕见

### 双刺毒花
- ModelId: `TWIN_STING_POISON_FLOWER`

- 费用: 1
- 类型: 攻击
- 效果: 失去1点生命 对所有敌人造成3点伤害 减少三层猩红腐败 削韧 1
- 稀有度: 罕见

### 双刺毒花+

- 费用: 1
- 类型: 攻击
- 效果: 失去1点生命 对所有敌人造成6点伤害 减少三层猩红腐败 削韧 1
- 稀有度: 罕见

### 荷莱露的撼地
- ModelId: `HOULOU_GROUND_SLAM`

- 费用: 2
- 类型: 攻击
- 效果: 对所有敌人造成5点伤害2次 削韧 3 下一回合开始时 对所有敌人造成6点伤害 削韧 4
- 稀有度: 罕见

### 荷莱露的撼地+

- 费用: 2
- 类型: 攻击
- 效果: 对所有敌人造成6点伤害2次 削韧 3 下一回合开始时 对所有敌人造成7点伤害 削韧 4
- 稀有度: 罕见

### 卡利亚贯刺
- ModelId: `CARIAN_PIERCER`

- 费用: 2
- 类型: 攻击
- 效果: 对所有敌人造成9点魔力属性伤害 并给予1层虚弱  削韧 2
- 稀有度: 罕见

### 卡利亚贯刺+

- 费用: 2
- 类型: 攻击
- 效果: 对所有敌人造成12点魔力属性伤害 并给予2层虚弱  削韧 2
- 稀有度: 罕见

### 帚星
- ModelId: `COMET_SHARD`

- 费用: 2
- 类型: 攻击
- 效果: 造成9点魔力属性伤害 获得2点魔力 这张牌的耗能减少1
- 稀有度: 罕见

### 帚星+

- 费用: 2
- 类型: 攻击
- 效果: 造成12点魔力属性伤害 获得3点魔力 这张牌的耗能减少1
- 稀有度: 罕见

### 黑夜彗星
- ModelId: `NIGHT_COMET`

- 费用: 1
- 类型: 攻击
- 效果: 造成8点魔力属性伤害 如果这张牌击杀了敌人 则对随机敌人造成4点魔力属性伤害2次
- 稀有度: 罕见

### 黑夜彗星+

- 费用: 1
- 类型: 攻击
- 效果: 造成11点魔力属性伤害 如果这张牌击杀了敌人 则对随机敌人造成5点魔力属性伤害2次
- 稀有度: 罕见

### 爆散结晶
- ModelId: `CRYSTAL_BURST`

- 费用: 1
- 类型: 攻击
- 效果: 造成4点魔力属性伤害2次 溅射
- 稀有度: 罕见

### 爆散结晶+

- 费用: 1
- 类型: 攻击
- 效果: 造成6点魔力属性伤害2次 溅射
- 稀有度: 罕见

### 罗蕾塔的大弓
- ModelId: `LORETTAS_GREATBOW`

- 费用: 3
- 类型: 攻击
- 效果: 造成24点魔力属性伤害 溅射
- 稀有度: 罕见

### 罗蕾塔的大弓+

- 费用: 3
- 类型: 攻击
- 效果: 造成32点魔力属性伤害 溅射
- 稀有度: 罕见

### 古老死亡怨魂
- ModelId: `ANCIENT_DEATHS_RANCOR`

- 费用: 2
- 类型: 攻击
- 效果: 随机对敌人造成2点魔力属性伤害6次 下回合开始时 再造成一次相同效果
- 稀有度: 罕见

### 古老死亡怨魂+

- 费用: 2
- 类型: 攻击
- 效果: 随机对敌人造成2点魔力属性伤害7次 下回合开始时 再造成一次相同效果
- 稀有度: 罕见

### 责罚荆棘
- ModelId: `PUNISHING_THORNS`

- 费用: 1
- 类型: 攻击
- 效果: 失去1点生命 对所有敌人造成9点魔法伤害 获得1点魔力
- 稀有度: 罕见

### 责罚荆棘+

- 费用: 1
- 类型: 攻击
- 效果: 失去1点生命 对所有敌人造成12点魔法伤害 获得2点魔力
- 稀有度: 罕见

### 狮子斩
- ModelId: `LION_CLAW`

- 费用: 2
- 类型: 攻击
- 效果: 造成20点伤害 削韧 6
- 稀有度: 罕见

### 狮子斩+

- 费用: 2
- 类型: 攻击
- 效果: 造成27点伤害 削韧 6
- 稀有度: 罕见

### 鲜血征收
- ModelId: `BLOOD_LEVY`

- 费用: 1
- 类型: 攻击
- 效果: 造成2点伤害3次 回复3点生命值 削韧 1
- 稀有度: 罕见

### 鲜血征收+

- 费用: 1
- 类型: 攻击
- 效果: 造成3点伤害3次 回复4点生命值 削韧 1
- 稀有度: 罕见

### 萨米尔冰风暴

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成1点伤害5次 削韧2 冻伤
- 稀有度: 稀有

### 萨米尔冰风暴+

- 费用: 1
- 类型: 攻击
- 效果: 对所有敌人造成1点伤害7次 削韧2 冻伤
- 稀有度: 稀有

### 彗星亚兹勒
- ModelId: `COMET_AZUR`

- 费用: 3
- 类型: 攻击
- 效果: 消耗抽牌堆任意数量牌 每消耗一张牌对敌人造成5点魔法属性伤害 失去三点力量 三点魔力 
- 稀有度: 稀有

### 彗星亚兹勒+

- 费用: 3
- 类型: 攻击
- 效果: 消耗抽牌堆任意数量牌 每消耗一张牌对敌人造成8点魔法属性伤害 失去三点力量 三点魔力 
- 稀有度: 稀有

### 图腾碑石
- ModelId: `TOTEM_TABLET`

- 费用: 0
- 类型: 攻击
- 效果: 对所有敌人造成10点伤害并击晕所有敌人 充能 10
- 稀有度: 稀有

### 图腾碑石+

- 费用: 0
- 类型: 攻击
- 效果: 对所有敌人造成13点伤害并击晕所有敌人 充能 8
- 稀有度: 稀有

## 技能牌

### 防御
- ModelId: `BASIC_DEFENSE`

- 费用: 1
- 类型: 技能
- 效果: 获得5点格挡
- 稀有度: 其他

### 防御+

- 费用: 1
- 类型: 技能
- 效果: 获得8点格挡
- 稀有度: 其他

### 小圆盾弹反
- ModelId: `SMALL_ROUND_SHIELD_PARRY`

- 费用: 1
- 类型: 技能
- 效果: 消耗 阻止接下来1次你将受到的伤害，击晕伤害来源，并将1张带消耗的致命一击加入手牌。
- 稀有度: 先古

### 小圆盾弹反+

- 费用: 1
- 类型: 技能
- 效果: 消耗 阻止接下来1次你将受到的伤害，击晕伤害来源，并将2张带消耗的致命一击加入手牌。
- 稀有度: 先古

### 火焰啊，净化
- ModelId: `FLAME_PURIFY`

- 费用: 1
- 类型: 技能
- 效果: 消除两层猩红腐败 获得8点格挡
- 稀有度: 普通

### 火焰啊，净化+

- 费用: 1
- 类型: 技能
- 效果: 消除三层猩红腐败 获得10点格挡
- 稀有度: 普通

### 休战
- ModelId: `FAT_BABY-TRUCE`
- 费用: 2
- 类型: 技能
- 效果: 获得20点格挡 下回合获得1点能量
- 稀有度: 普通

### 休战+
- 费用: 2
- 类型: 技能
- 效果: 获得23点格挡 下回合获得2点能量
- 稀有度: 普通

### 恫吓
- ModelId: `INTIMIDATION`

- 费用: 1
- 类型: 技能
- 效果: 给予一层虚弱 获得8点格挡
- 稀有度: 普通

### 恫吓+

- 费用: 1
- 类型: 技能
- 效果: 给予两层虚弱 获得10点格挡
- 稀有度: 普通

### 轻装上阵
- ModelId: `TRAVEL_LIGHT`

- 费用: 1
- 类型: 技能
- 效果: 获得8点格挡 丢弃任意张牌 每丢弃一张牌获得4点格挡
- 稀有度: 普通

### 轻装上阵+

- 费用: 1
- 类型: 技能
- 效果: 获得10点格挡 丢弃任意张牌 每丢弃一张牌获得5点格挡
- 稀有度: 普通

### 决心
- ModelId: `DETERMINATION`

- 费用: 1
- 类型: 技能
- 效果: 下一张攻击牌造成100%额外伤害
- 稀有度: 普通

### 决心+

- 费用: 0
- 类型: 技能
- 效果: 下一张攻击牌造成100%额外伤害
- 稀有度: 普通

### 岩石剑
- ModelId: `ROCK_BLADE`

- 费用: 1
- 类型: 技能
- 效果: 下一张攻击牌额外削韧 3
- 稀有度: 普通

### 岩石剑+

- 费用: 0
- 类型: 技能
- 效果: 下一张攻击牌额外削韧 4
- 稀有度: 普通

### 寻找
- ModelId: `SEARCH`

- 费用: 1
- 类型: 技能
- 效果: 抽牌直到抽到非攻击牌 消耗2张牌
- 稀有度: 普通

### 寻找+

- 费用: 0
- 类型: 技能
- 效果: 抽牌直到抽到非攻击牌 消耗2张牌
- 稀有度: 普通

### 猩红代价
- ModelId: `SCARLET_COST`

- 费用: 0 
- 类型: 技能
- 效果: 抽三张牌 获得1层猩红腐败
- 稀有度: 普通

### 猩红代价+

- 费用: 0 
- 类型: 技能
- 效果: 抽四张牌 获得1层猩红腐败
- 稀有度: 普通

### 夸耀咆哮
- ModelId: `BATTLE_CRY`

- 费用: 1
- 类型: 技能
- 效果: 获得1点力量 抽两张牌
- 稀有度: 普通

### 夸耀咆哮+

- 费用: 1
- 类型: 技能
- 效果: 获得1点力量 抽三张牌
- 稀有度: 普通

### 游刃有余
- ModelId: `EASY_HANDLING`

- 费用: 1
- 类型: 技能
- 效果: 抽三张牌 消耗1张牌
- 稀有度: 罕见

### 游刃有余+

- 费用: 1
- 类型: 技能
- 效果: 抽四张牌 消耗1张牌
- 稀有度: 罕见

### 忍耐
- ModelId: `ENDURE`

- 费用: 1
- 类型: 技能
- 效果: 本回合受到的伤害降低50%
- 稀有度: 罕见

### 忍耐+

- 费用: 1
- 类型: 技能
- 效果: 本回合受到的伤害降低50% 获得5点格挡
- 稀有度: 罕见

### 潜雾猛禽

- 费用: 1
- 类型: 技能
- 效果: 本回合受到的伤害降低25% 在弃牌堆中挑选一张卡加入手中
- 稀有度: 罕见

### 潜雾猛禽+

- 费用: 1
- 类型: 技能
- 效果: 本回合受到的伤害降低25% 在弃牌堆中挑选一张卡加入手中
- 稀有度: 罕见

### 杰克的酒
- ModelId: `JACK_WINE`

- 费用: 1
- 类型: 技能
- 效果: 回复5点生命 下回合获得1点能量 
- 稀有度: 罕见

### 杰克的酒+

- 费用: 1
- 类型: 技能
- 效果: 回复8点生命 下回合获得1点能量 
- 稀有度: 罕见

### 卸力
- ModelId: `UNBURDEN`

- 费用: 0
- 类型: 技能
- 效果: 失去一点力量 获得2点能量
- 稀有度: 罕见

### 卸力+

- 费用: 0
- 类型: 技能
- 效果: 失去一点力量 获得3点能量
- 稀有度: 罕见
 
### 赊账
- ModelId: `SCARLET_CREDIT`

- 费用: 0
- 类型: 技能
- 效果: 获得2层猩红腐败 获得2点能量
- 稀有度: 罕见

### 赊账+

- 费用: 0
- 类型: 技能
- 效果: 获得2层猩红腐败 获得3点能量
- 稀有度: 罕见

### 深具睿智
- ModelId: `PROFOUND_WISDOM`

- 费用: 1
- 类型: 技能
- 效果: 获得2点魔力 抽两张牌
- 稀有度: 罕见

### 深具睿智+

- 费用: 1
- 类型: 技能
- 效果: 获得4点魔力 抽两张牌
- 稀有度: 罕见

### 巨剑阵
- ModelId: `GLINTBLADE_PHALANX`

- 费用: 3
- 类型: 技能
- 效果: 召唤3个辉石巨剑 下一回合 每个辉石巨剑对随机敌人造成7点魔力属性伤害 削韧 5
- 稀有度: 罕见

### 巨剑阵+

- 费用: 2
- 类型: 技能
- 效果: 召唤3个辉石巨剑 下一回合 每个辉石巨剑对随机敌人造成7点魔力属性伤害 削韧 5
- 稀有度: 罕见

### 切腹
- ModelId: `SEPPUKU`

- 费用: 2
- 类型: 技能
- 效果: 失去5点生命 消耗手中所有的牌 每消耗一张牌 获得1点力量和1点魔力 消耗
- 稀有度: 罕见

### 切腹+

- 费用: 2
- 类型: 技能
- 效果: 失去2点生命 消耗手中所有的牌 每消耗一张牌 获得1点力量和1点魔力 消耗
- 稀有度: 罕见

### 孤注一掷
- ModelId: `ALL_IN`

- 费用: X
- 类型: 技能
- 效果: 消耗一张牌 打出X次被消耗的牌
- 稀有度: 罕见

### 孤注一掷+

- 费用: X
- 类型: 技能
- 效果: 消耗一张牌 打出X+1次被消耗的牌
- 稀有度: 罕见

### 掠夺之火
- ModelId: `PLUNDERING_FIRE`

- 费用: 1
- 类型: 技能
- 效果: 消耗 本回合 你通过攻击牌造成未被格挡的伤害时 回复等量生命
- 稀有度: 稀有

### 掠夺之火+

- 费用: 0
- 类型: 技能
- 效果: 消耗 本回合 你通过攻击牌造成未被格挡的伤害时 回复等量生命
- 稀有度: 稀有

### 好运
- ModelId: `GOOD_LUCK`

- 费用: 0
- 类型: 技能
- 效果: 消耗 获得1点力量 1点魔力 获得1能量 抽一张卡 
- 稀有度: 稀有

### 好运+

- 费用: 0
- 类型: 技能
- 效果: 消耗 获得1点力量 1点魔力 获得2能量 抽两张卡 
- 稀有度: 稀有

### 别急，等我启动
- ModelId: `WAIT_FOR_ME_TO_START`

- 费用: 3
- 类型: 技能
- 效果: 固有 消耗 两回合内不会受到任何伤害 获得两层虚弱 手牌保留两轮 
- 稀有度: 稀有

### 别急，等我启动+

- 费用: 3
- 类型: 技能
- 效果: 固有 消耗 三回合内不会受到任何伤害 获得三层虚弱 手牌保留三轮
- 稀有度: 稀有

### 卡利亚式奉还
- ModelId: `CARIAN_RETRIBUTION`

- 费用: 1
- 类型: 攻击
- 效果: 阻止下一次你受到的伤害，击晕伤害来源，并对攻击者造成等量的魔力属性伤害。
- 稀有度: 稀有

### 卡利亚式奉还+

- 费用: 1
- 类型: 攻击
- 效果: 阻止接下来2次你受到的伤害，击晕伤害来源，并对攻击者造成等量的魔力属性伤害。
- 稀有度: 稀有

### 回归性原理
- ModelId: `FAT_BABY-RETURN_PRINCIPLE`
- 费用: 2
- 类型: 技能
- 效果: 消耗 消除自身所有负面效果 获得2点力量和2点魔力
- 稀有度: 稀有

### 回归性原理+
- 费用: 1
- 类型: 技能
- 效果: 消耗 消除自身所有负面效果 获得2点力量和2点魔力
- 稀有度: 稀有

## 能力

### 余火
- ModelId: `EMBER`

- 费用: 1
- 类型: 能力
- 效果: 获得1点力量 1点魔力 2点最大生命 攻击牌额外削韧 1
- 稀有度: 罕见

### 余火+

- 费用: 1
- 类型: 能力
- 效果: 获得2点力量 1点魔力 3点最大生命 攻击牌额外削韧 1
- 稀有度: 罕见

### 腐败力量
- ModelId: `CORRUPTION_STRENGTH`

- 费用: 1
- 类型: 能力
- 效果: 获得2点力量 获得猩红腐败2层 
- 稀有度: 罕见

### 腐败力量+

- 费用: 1
- 类型: 能力
- 效果: 获得5点力量 获得猩红腐败2层
- 稀有度: 罕见

### 腐败感知
- ModelId: `CORRUPTION_SENSE`

- 费用: 1
- 类型: 能力
- 效果: 获得猩红腐败时 抽1张牌
- 稀有度: 罕见

### 腐败感知+

- 费用: 1
- 类型: 能力
- 效果: 获得猩红腐败时 抽2张牌
- 稀有度: 罕见

### 乘胜追击
- ModelId: `VICTORY_RUSH`

- 费用: 2
- 类型: 能力
- 效果: 击晕敌人时 获得1费 打出的下一张攻击牌会被额外打出一次
- 稀有度: 罕见

### 乘胜追击+

- 费用: 1
- 类型: 能力
- 效果: 击晕敌人时 获得1费 打出的下一张攻击牌会被额外打出一次
- 稀有度: 罕见

### 猩红诱惑
- ModelId: `SCARLET_TEMPTATION`

- 费用: 2
- 类型: 能力
- 效果: 每回合额外抽2张牌并从手牌中消耗一张牌 获得猩红腐败3层
- 稀有度: 稀有

### 猩红诱惑+

- 费用: 2
- 类型: 能力
- 效果: 固有 每回合额外抽2张牌并从手牌中消耗一张牌 获得猩红腐败3层
- 稀有度: 稀有

### 魔法之境
- ModelId: `MAGIC_REALM`

- 费用: 2
- 类型: 能力
- 效果: 每回合开始阶段 获得3点魔力并获得10点格挡
- 稀有度: 稀有

### 魔法之境+

- 费用: 1
- 类型: 能力
- 效果: 每回合开始阶段 获得4点魔力并获得10点格挡
- 稀有度: 稀有

### 垃圾王的庇佑
- ModelId: `GARBAGE_KING_BLESSING`

- 费用: 2
- 类型: 能力
- 效果: 虚无 在回合开始时 获得4点力量 获得猩红腐败3层
- 稀有度: 稀有

### 垃圾王的庇佑+

- 费用: 2
- 类型: 能力
- 效果: 虚无 在回合开始时 获得5点力量 获得猩红腐败3层
- 稀有度: 稀有

### 好胜心
- ModelId: `WILL_TO_WIN`

- 费用: 3
- 类型: 能力
- 效果: 本场战斗生命值不会低于1
- 稀有度: 稀有

### 好胜心+

- 费用: 3
- 类型: 能力
- 效果: 保留 本场战斗生命值不会低于1
- 稀有度: 稀有

### 破势如潮
- ModelId: ``BREAKING_MOMENTUM``

- 费用: 3
- 类型: 能力
- 效果: 每造成一个敌人眩晕 获得1点额外削韧
- 稀有度: 稀有

### 破势如潮+

- 费用: 3
- 类型: 能力
- 效果: 每造成一个敌人眩晕 获得1点额外削韧和1点力量
- 稀有度: 稀有
