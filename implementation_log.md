...
- `CHARGED_ATTACK` / 蓄力攻击：2 费基础攻击，15 伤害，削韧 5；升级 19 伤害、削韧 6。
- `CHARGED_HEAVY_ATTACK` / 蓄力重击：1 费先古攻击，15 伤害，削韧 5，下次受伤减免 75%；升级 19 伤害、削韧 6。
...
- `CARIAN_SLICER` / 卡利亚迅剑：1 费普通攻击，6 魔法伤害，打出后将这张牌返回手牌，无削韧；升级 9 伤害。
...
- 奖池已实现 **63** 张 + 衍生 `ELEMENT_FLASK`；`SPIRAL_GLINTSTONE` 暂缓落地，待打包更新后加回。
- `CHARGED_HEAVY_ATTACK` 由弃牌回手改为 **下次受伤减免 75%**；`CHARGED_ATTACK` 升级改为 19 伤 / 6 削韧。
- 已同步 `cards.md`、卡牌状态 Canvas、`tests/card_expectations.json` 与 zhs 本地化标点。
...
- 人物战斗贴图、战斗动画、能量图标。
- 卡牌正式卡面贴图。
  - 生图规范见根目录 `prompt.md`：构图保真（原帧裁切 + 轻修），**统一色调参考 `blood_slash.png`**；后处理可用 `scripts/grade_card_portrait.py`。
- 猩红腐败相关联动:
